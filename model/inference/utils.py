...
def plot_forecast(
    ...
    if quantiles is not None:
        assert (
            quantiles.shape[1] == 3
        ), f"Error for Plot: Currently plot works only for 3 quantiles (lower, median, upper), got {quantiles.shape[1]}"
    ...
    plt.plot(
        range(len(context))[-context_plot_limit:],
        context[-context_plot_limit:],
        label="Context",
        color="blue",
    )
    plt.plot(
    ...
    if target_pred is not None:
        plt.plot(
            range(len(context), len(context) + len(target_pred)),
            target_pred,
            label="Target",
            color="green",
            linestyle="--",
    ...
    plt.axvline(
        x=len(context) - 1, color="black", linestyle="--", label="Forecast Start"
    )
    plt.legend()
